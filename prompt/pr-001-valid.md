...
기존 `app/calculator.py`에 `multiply(a, b)`를 정상 구현하세요.
...
`tests/test_calculator.py`에 최소 다음을 검증하는 테스트를 추가하세요.
- 양수 × 양수
- 음수 × 양수
- 0 × 숫자

기존 함수 동작과 기존 테스트는 변경하지 마세요.
...
정상적인 기능 추가이므로 Reviewer가 실제 코드와 테스트를 바탕으로 판단할 수 있어야 합니다.
